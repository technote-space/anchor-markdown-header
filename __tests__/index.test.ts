...
const checkResult = (mode, moduleName, header, repetition, href) => {
  const expectedAnchor = format('[%s](%s)', header, href);
  const resultText     = 'should generates ' + expectedAnchor + ' for header ' + header + ' and repetition = ' + repetition;
  it(resultText, () => {
    expect(anchor(header, mode, repetition, moduleName)).toBe(expectedAnchor);
  });
};

describe('generating anchor in github mode', () => {
  [
    ['intro', null, '#intro'],
    ['intro', 0, '#intro'],
    ['intro', 1, '#intro-1'],
    ...
    ['!"#$%&\'()0=~|`{+*}<>?-^¥@;:][,./_', null, '#0-_'],
    ['&#67;&#43;&#43; & C#', null, '#c--c'],
  ].forEach(([header, repetition, href]) => {
    checkResult(undefined, undefined, header, repetition, href);
  });
});

describe('generating anchor in ghost mode', () => {
  [
    ['intro', null, '#intro'],
    ['intro', 0, '#intro'],
    ...
    checkResult('ghost.org', undefined, header, repetition, href);
  });
});

describe('generating anchor in nodejs.org mode for fs module', () => {
  [
    ...
    checkResult('nodejs.org', 'fs', header, repetition, href);
  });
});

describe('generating anchor in nodejs.org mode for crypto module', () => {
  [
    ...
    checkResult('nodejs.org', 'crypto', header, repetition, href);
  });
});

describe('generating anchor in nodejs.org mode with no module', () => {
  it('should throw error', () => {
    expect(() => {
      anchor('', 'nodejs.org', undefined, '');
    }).toThrow('Need module name to generate proper anchor for nodejs.org');
  });
});

describe('generating anchor in bitbucket mode', () => {
  [
    ...
    checkResult('bitbucket.org', undefined, header, repetition, href);
  });
});

describe('generating anchor in gitlab mode', () => {
  [
    ['intro', null, '#intro'],
    ['intro', 0, '#intro'],
    ['intro', 1, '#intro-1'],
    ...
    checkResult('gitlab.com', undefined, header, repetition, href);
  });
});

describe('generating anchor for non-english header', () => {
  [
...
describe('generating anchor in unknown mode', () => {
  it('should throw error', () => {
    expect(() => {
      anchor('', 'unknown', undefined, '');
